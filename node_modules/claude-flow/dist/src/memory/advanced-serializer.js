export class AdvancedSerializer {
    constructor(options = {}){
        ...
        this.typeHandlers = new Map([
            [
                'Date',
                this._serializeDate.bind(this)
            ],
            [
                'Map',
                this._serializeMap.bind(this)
            ],
            [
                'Set',
                this._serializeSet.bind(this)
            ],
            [
                'RegExp',
                this._serializeRegExp.bind(this)
            ],
            [
                'Error',
                this._serializeError.bind(this)
            ],
            [
                'BigInt',
                this._serializeBigInt.bind(this)
            ],
            [
                'ArrayBuffer',
                this._serializeArrayBuffer.bind(this)
            ],
            [
                'TypedArray',
                this._serializeTypedArray.bind(this)
            ],
            [
                'Function',
                this._serializeFunction.bind(this)
            ],
            [
                'Symbol',
                this._serializeSymbol.bind(this)
            ]
        ]);
        this.deserializers = new Map([
            [
                '__Date__',
                this._deserializeDate.bind(this)
            ],
            [
                '__Map__',
                this._deserializeMap.bind(this)
            ],
            [
                '__Set__',
                this._deserializeSet.bind(this)
            ],
            [
                '__RegExp__',
                this._deserializeRegExp.bind(this)
            ],
            [
                '__Error__',
                this._deserializeError.bind(this)
            ],
            [
                '__BigInt__',
                this._deserializeBigInt.bind(this)
            ],
            [
                '__ArrayBuffer__',
                this._deserializeArrayBuffer.bind(this)
            ],
            [
                '__TypedArray__',
                this._deserializeTypedArray.bind(this)
            ],
            [
                '__Function__',
                this._deserializeFunction.bind(this)
            ],
            [
                '__Symbol__',
                this._deserializeSymbol.bind(this)
            ],
            [
                '__undefined__',
                ()=>undefined
    ...
    _serializeValue(value, context) {
        ...
        if (value === Infinity) {
            return {
                __type__: '__Infinity__'
            };
        }
        if (value === -Infinity) {
            return {
                __type__: '__-Infinity__'
            };
        }
        ...
        return {
            __unknown__: this._safeStringify(value)
        };
    }
    ...
    _serializeMap(map) {
        return {
            ...
            entries: Array.from(map.entries()).map(([k, v])=>[
                    this._serializeValue(k, {
                        depth: 0,
                        seen: new WeakSet()
                    }),
                    this._serializeValue(v, {
                        depth: 0,
                        seen: new WeakSet()
                    })
                ])
    ...
    _serializeSet(set) {
        return {
            __type__: '__Set__',
            values: Array.from(set).map((v)=>this._serializeValue(v, {
                    depth: 0,
                    seen: new WeakSet()
                }))
        };
    ...
    _serializeError(error) {
        return {
            ...
            stack: error.stack,
            cause: error.cause ? this._serializeValue(error.cause, {
                depth: 0,
                seen: new WeakSet()
            }) : undefined
        };
...
//# sourceMappingURL=advanced-serializer.js.map